...
.DESCRIPTION
    ...
    "Last user" lookups are skipped to reduce API calls.
#>
...
$activeComputers = $computers | Where-Object { $_.Body -and $_.Body.Status -eq 'Active' }
if ($computers.Count -ne $activeComputers.Count) {
    Write-Host "Filtered out $($computers.Count - $activeComputers.Count) non-active computers." -ForegroundColor Gray
}
...
foreach ($compResource in $activeComputers) {
